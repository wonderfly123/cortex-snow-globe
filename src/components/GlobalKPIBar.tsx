...
import TimeSlider, { MONTH_LABELS } from './AnalyticsPanel/TimeSlider'
...
export function GlobalKPIBar() {
  ...
  }, [salesTrendData, setSalesTrendData])

  const kpis = useMemo(() => {
    if (salesTrendData) {
      const filteredMonths = new Set(MONTH_LABELS.slice(timeRange[0], timeRange[1] + 1))
    ...
    const avgOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0
    return { totalRevenue, totalOrders, avgOrderValue }
  }, [cities, salesTrendData, timeRange])
